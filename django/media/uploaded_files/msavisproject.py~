...
PATH = "/home/marta/Dokumenty/praktyki/django/media/"
...
def pred_secondary_structure(seqDict):
  ...
  for number_seq,seq in enumerate(sequences, start=0):
    ...
    output_handle = open(PATH+"uploaded_files/psipred/current_seq_temp.fasta", "w")
    try:
    ...
    # calling runpsipred
    p = sub.Popen([PATH+"uploaded_files/psipred/runpsipred_single", PATH+"uploaded_files/psipred/current_seq_temp.fasta"], cwd=PATH+"uploaded_files/psipred/", stdin=sub.PIPE, stdout=sub.PIPE, stderr=sub.STDOUT)
    ...
    try:
      file_tmp=open(PATH+"uploaded_files/psipred/current_seq_temp.ss", "r")
    except Exception:
  ...
  # removing temporary files 
  l = [PATH+"uploaded_files/psipred/current_seq_temp.fasta",
  PATH+"uploaded_files/psipred/current_seq_temp.ss",
  PATH+"uploaded_files/psipred/current_seq_temp.ss2",
  PATH+"uploaded_files/psipred/current_seq_temp.horiz"]
...
def readKD():
	...
	KD={}
	f=open(PATH+"uploaded_files/KD.scale","r")
	lines=f.readlines()
