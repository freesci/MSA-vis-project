...
def inpout():
    ...
    parser.add_argument('inp', type=str, nargs='*', help='file with MSA in fasta, phylip (4 or interleaved) or clustal (<=2.0) format')
    ...
    parser.add_argument('-a', '--aminoacids', type=int, nargs='?',default=100,
                        help='number of aminoacids in one row in graph. Enter the number greater than 0 (Default 100).')

    parser.add_argument('-f', '--format', type=str, nargs='?', default='fasta',
			help='input format. Use \'fasta\', \'phylip\' or \'clustal\' (default fasta)')

    parser.add_argument('-p', '--psipred', type=str, nargs='?',default="runpsipred", help='decision of run program with database or without (default with database). Use \'runpsipred\' (with database) or \'runpsipred_single\' (without database).')
    ...
    if args.format!='fasta' and args.format!='phylip' and args.format!='clustal':
        print colored("Error: Wrong format specified\n----------------------------------",'red')
        parser.print_help()
        exit()

    if args.psipred == None:
	args.psipred = "runpsipred"

    if args.psipred!='runpsipred' and args.psipred!='runpsipred_single':
        print colored("Error: Wrong psipred version specified\n----------------------------------",'red')
        parser.print_help()
        exit()


    return args.inp, args.output, args.aminoacids, args.format, args.psipred, parser


inp,out,nam,format,psipred,parser=inpout()
...
def consensus(seqDict):
	...
	for i in xrange(len(seqDict[id[0]])):
		for key in id:
			if seqDict[key][i]!="-" and upper(seqDict[key][i])!="X":	#X is treated like a gap
				...
				n+=1
		if n==0:
			print "Error: Wrong MSA. There is no non-gap letter on position "+str(i)+" (X is treated like a gap)"
        		exit(1)
...
def pred_secondary_structure(seqDict,psipredchoice):
  ...
  for number_seq,seq in enumerate(sequences, start=0):
    ...
    # calling runpsipred
    p = sub.Popen([psipredchoice, "current_seq_temp.fasta"], stdin=sub.PIPE, stdout=sub.PIPE, stderr=sub.STDOUT)
  ...
  # removing temporary files
  l = ["current_seq_temp.fasta",
  "current_seq_temp.ss",
  "current_seq_temp.ss2",
  "current_seq_temp.horiz"]
...
arguments:
	...
	col - number of columns (default=100)
"""
...
if __name__ == "__main__":
  ...
  stru = pred_secondary_structure(dictionary, './'+psipred)
